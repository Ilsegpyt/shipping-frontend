...
const containerSizes = [
    { value: 1, label: 'Dry 20 Standard', apiValue: 'Dry20Standard' },
    { value: 2, label: 'Dry 40 Standard', apiValue: 'Dry40Standard' },
    { value: 3, label: 'Dry 40 High', apiValue: 'Dry40High' },
    { value: 4, label: 'Dry 45 High', apiValue: 'Dry45High' },
    { value: 5, label: 'Reefer 20 Standard', apiValue: 'Reefer20Standard' },
    { value: 6, label: 'Reefer 40 High', apiValue: 'Reefer40High' },
    { value: 7, label: 'Open Top 20', apiValue: 'OpenTop20' },
    { value: 8, label: 'Open Top 40', apiValue: 'OpenTop40' },
    { value: 9, label: 'Open Top 40 High', apiValue: 'OpenTop40High' },
    { value: 10, label: 'Flat 40 Standard', apiValue: 'Flat40Standard' },
    { value: 11, label: 'Flat 40 High', apiValue: 'Flat40High' },
    { value: 12, label: 'Flat 20', apiValue: 'Flat20' },
    { value: 13, label: 'Tank 20', apiValue: 'Tank20' },
    { value: 14, label: 'Tank 40', apiValue: 'Tank40' },
];
...
export default function ScheduleEdit() {
        ...
        const loadSchedule = async () => {
            try {
                ...
                const containerSize = containerSizes.find(
                    (container) =>
                        container.apiValue === data.containerSize
                );
                ...
                setForm({
                    ...
                    origin: data.origin ?? '',
                    departurePortCode:
                        data.departurePortCode ?? '',
                    departureCountry:
                        data.departureCountry ?? '',

                    destination: data.destination ?? '',
                    arrivalPortCode:
                        data.arrivalPortCode ?? '',
                    arrivalCountry:
                        data.arrivalCountry ?? '',
                    ...
                    rateAmount:
                        data.rateAmount !== null &&
                            data.rateAmount !== undefined
                            ? String(data.rateAmount)
                            : '',
                    rateRemarks: data.rateRemarks ?? '',

                    freeTimeAtPOD:
                        data.freeTimeAtPOD !== null &&
                            data.freeTimeAtPOD !== undefined
                            ? String(data.freeTimeAtPOD)
                            : '0',
                    freeTimeAtPOL:
                        data.freeTimeAtPOL !== null &&
                            data.freeTimeAtPOL !== undefined
                            ? String(data.freeTimeAtPOL)
                            : '0',

                    transshipmentData:
                        data.transshipmentData ?? '',
                    notes: data.notes ?? '',
                });
            } catch (err) {
                console.error(
    ...
    const validateForm = () => {
        ...
        const requiredFields = [
            ...
            [
                'departurePortCode',
                'Departure port code is required.',
            ],
            [
                'departureCountry',
    ...
    const handleSubmit = async (event) => {
        ...
        const scheduleData = {
            ...
            departurePortCode:
                form.departurePortCode.trim(),
            departureCountry:
                form.departureCountry.trim(),

            destination: form.destination.trim(),
            arrivalPortCode:
                form.arrivalPortCode.trim(),
            arrivalCountry:
                form.arrivalCountry.trim(),
            ...
            rateCurrency:
                form.rateCurrency.trim().toUpperCase(),
                ...
                <div className="mb-6 flex items-start justify-between">
                    <div>
                        ...
                        <p className="mt-1 text-sm text-blue-900/70">
                            Update the shipping schedule and route rate.
                        </p>
                    </div>
                </div>
